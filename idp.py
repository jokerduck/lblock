...
class Cipher(AbstractCipher):
    ...
    def createSTP(self, stp_filename, parameters):
        ...
        with open(stp_filename, 'w') as stp_file:
            ...
            #enable = ["enable{}".format(i) for i in range(16)]
            
            enable = ["enable{}".format(i) for i in range(4)]
            sumhw=["sumhw{}".format(i) for i in range(2)]
            ...
            stpcommands.setupVariables(stp_file, enable, 8)
            stpcommands.setupVariables(stp_file, sumhw, 8)

            stpcommands.setupWeightComputationSum(stp_file, weight, w, 16)

            self.SBOX_ACT_ASSERT(stp_file)
            command="ASSERT(sumhw0 = 0bin00000001);\n"
            command="ASSERT(sumhw1 = 0bin00000001);\n"
            for i in range(wordsize // 4):
                flagx0_in_4_bit = "{0}[{1}:{1}]@{0}[{2}:{2}]@" \
                         "{0}[{3}:{3}]@{0}[{4}:{4}]".format(
                            x[0],
                            wordsize - 1 - 4 * i - 0,
                            wordsize - 1 - 4 * i - 1,
                ...
                #command += "ASSERT(FLAG[{}] = 0bin1);\n".format(s_in_4_bit, s_out_4_bit_G0, s_out_4_bit_G1)
                command += "ASSERT({0}[{2}:{2}] = (IF {1} = 0bin0000 " \
                                            "THEN 0bin0 " \
                                      "ELSE 0bin1 ENDIF));\n".format(
                                        enable[0],flagx0_in_4_bit,i)
                command += "ASSERT({0}[{2}:{2}] = (IF {1} = 0bin0000 " \
                                            "THEN 0bin0 " \
                                      "ELSE 0bin1 ENDIF));\n".format(
                                        enable[1],flagy0_in_4_bit,i)
                command += "ASSERT({0}[{2}:{2}] = (IF {1} = 0bin0000 " \
                                            "THEN 0bin0 " \
                                      "ELSE 0bin1 ENDIF));\n".format(
                                        enable[2],flagxend_in_4_bit,i)
                command += "ASSERT({0}[{2}:{2}] = (IF {1} = 0bin0000 " \
                                            "THEN 0bin0 " \
                                      "ELSE 0bin1 ENDIF));\n".format(
                                        enable[3],flagyend_in_4_bit,i)
                
            inbox = stpcommands.getWeightString([enable[0],enable[1]], wordsize // 4, 0, sumhw[0])
            inbox += '\n'
            command += inbox
            inbox = stpcommands.getWeightString([enable[2],enable[3]], wordsize // 4, 0, sumhw[1])
            inbox += '\n'
            command += inbox
